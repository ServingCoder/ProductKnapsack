...
def adjacent(packing, rnd):
  # creates a neighbour solution
  n = len(packing)
  result = np.copy(packing)
  i = rnd.randint(n)
  # randomly switch one item in the knapsack from working or not
  if result[i] == 0:
    result[i] = 1
  elif result[i] == 1:
    result[i] = 0
...
def solve(n_items, rnd, valus, sizes, max_size, \
  ...
  #get the current value and size of this permutation, if over, return 0
